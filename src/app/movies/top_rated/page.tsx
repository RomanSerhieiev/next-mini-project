...
const TopRatedMoviesPage: FC<IProps> = async ({searchParams}) => {
    ...
    if (searchParams.page && +searchParams.page <= 500) {
        ...
        redirect(`/movies/top_rated?page=500`);
    } else {
        page = '1';
    }

    const {results: movies, total_pages} = await movieService.topRated(page);
    if (movies.length === 0) {
        redirect(`/movies/top_rated?page=${total_pages}`);
    }
