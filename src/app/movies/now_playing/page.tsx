...
export const metadata: Metadata = {
    title: 'Now Playing Movies — The Movie Database (TMDB)',
    description: 'The Movie Database (TMDB) is a popular, user editable database for movies and TV shows.',
};

const NowPlayingMoviesPage: FC<IProps> = async ({searchParams}) => {
    ...
    if (searchParams.page && +searchParams.page <= 500) {
        ...
        redirect(`/movies/now_playing?page=500`);
    } else {
        page = '1';
    }

    const {results: movies, total_pages} = await movieService.nowPlaying(page);
    if (movies.length === 0) {
        redirect(`/movies/now_playing?page=${total_pages}`);
    }
